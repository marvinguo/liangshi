...
void SlidingWidget::setVerticalMode(bool vertical)
{
	m_vertical=vertical;
}

void SlidingWidget::setSpeed(int speed)
{
	m_speed = speed;
}

void SlidingWidget::setAnimation(enum QEasingCurve::Type animationtype)
{
	m_animationtype = animationtype;
}

void SlidingWidget::setWrap(bool wrap)
{
	m_wrap=wrap;
}

void SlidingWidget::slideInNext()
{
...
void SlidingWidget::slideInPrev()
{
	int now=currentIndex();
	if (m_wrap||(now>0))
		slideInIdx(now-1);
...
void SlidingWidget::slideInIdx(int idx, enum t_direction direction)
{
...
void SlidingWidget::slideInWgt(QWidget * newwidget, enum t_direction  direction)
{
	if (m_active)
	{
	...
	connect(animgroup, &QParallelAnimationGroup::finished, this, &SlidingWidget::animationDoneSlot);
	m_next=next;
	m_now=now;
	m_active=true;
	animgroup->start();
...
void SlidingWidget::animationDoneSlot(void)
{
