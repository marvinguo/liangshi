...
/*!
    ...
    the identifier is Q_EMITted and when the request is finished, the
    requestFinished() signal is Q_EMITted with the identifier and a bool
    ...
    For this example the following sequence of signals is Q_EMITted
...
/*!
    ...
    This signal is Q_EMITted when the state of the QHttp object changes.
...
/*!
    ...
    This signal is Q_EMITted when the HTTP header of a server response
...
/*!
    ...
    This signal is Q_EMITted when there is new response data to read.

    If you specified a device in the request where the data should be
    written to, then this signal is \e not Q_EMITted; instead the data
...
/*!
    ...
    This signal is Q_EMITted when this object sends data to a HTTP
...
/*!
    ...
    This signal is Q_EMITted when this object reads data from a HTTP
...
/*!
    ...
    This signal is Q_EMITted when processing the request identified by
    \a id starts.
...
/*!
    ...
    This signal is Q_EMITted when the last pending request has finished;
    (it is Q_EMITted after the last request's requestFinished() signal).
...
/*!
    ...
    This signal can be Q_EMITted when a \a proxy that requires
...
/*!
    ...
    This signal can be Q_EMITted when a web server on a given \a hostname and \a
...
/*!
    ...
    error argument \c true is Q_EMITted. For all other requests that are
    affected by the abort(), no signals are Q_EMITted.

    Since this slot also deletes the scheduled requests, there are no
    requests left and the done() signal is Q_EMITted (with the \c error
    argument \c true).
...
/*!
    ...
    Q_EMITted. When it is finished the requestFinished() signal is
    Q_EMITted.
...
/*!
    ...
    If the IO device \a to is 0 the readyRead() signal is Q_EMITted
    ...
    delete it when the requestFinished() signal is Q_EMITted).
...
int QHttpPrivate::addRequest(QHttpRequest *req)
{
    ...
    if (pending.count() == 1) {
        // don't Q_EMIT the requestStarted() signal before the id is returned
...
void QHttpPrivate::finishedWithSuccess()
{
    ...
    Q_EMIT q->requestFinished(r->id, false);
    if (hasFinishedWithError) {
        // we recursed and changed into an error. The finishedWithError function
        // below has Q_EMITted the done(bool) signal and cleared the queue by now.
        return;
    }
...
void QHttpPrivate::finishedWithError(const QString &detail, int errorCode)
{
    ...
    if (!r->finished) {
        r->finished = true;
        Q_EMIT q->requestFinished(r->id, true);
    }

    while (!pending.isEmpty())
        delete pending.takeFirst();
    Q_EMIT q->done(hasFinishedWithError);
}
...
void QHttpPrivate::_q_slotBytesWritten(qint64 written)
{
    Q_Q(QHttp);
    bytesDone += written;
    Q_EMIT q->dataSendProgress(bytesDone, bytesTotal);
    postMoreData();
}
...
void QHttpPrivate::_q_slotReadyRead()
{
    ...
    while (readHeader) {
        ...
        if (statusCode == 401 || statusCode == 407) { // (Proxy) Authentication required
            ...
            if (priv->phase == QHttpAuthenticatorPrivate::Done) {
...
#ifndef QT_NO_NETWORKPROXY
                if (statusCode == 407) {
                    //need to Q_EMIT a QAuthenticator to maintain source compatibility
                    QAuthenticator qauthClean = auth->toQAuthenticator();
                    QAuthenticator qauthToQ_EMIT = qauthClean;
                    Q_EMIT q->proxyAuthenticationRequired(proxy, &qauthToQ_EMIT);
                    if (qauthClean != qauthToQ_EMIT) {
...
#endif
                {
                    ...
                    QAuthenticator qauthClean = auth->toQAuthenticator();
                    QAuthenticator qauthToQ_EMIT = qauthClean;
                    Q_EMIT q->authenticationRequired(hostName, port, &qauthToQ_EMIT);
                    if (qauthClean != qauthToQ_EMIT) {
                        //user changed something, copy back (which will reset our state)
                        *auth = qauthToQ_EMIT;
                    }
        ...
        if (response.statusCode() != 100) {
            ...
            if (!repost)
                Q_EMIT q->responseHeaderReceived(response);
        ...
        if (arr && !repost) {
            ...
            if (toDevice) {
                ...
                if (response.hasContentLength())
                    Q_EMIT q->dataReadProgress(bytesDone, response.contentLength());
                else
                    Q_EMIT q->dataReadProgress(bytesDone, 0);
            } else {
...
#endif
                if (response.hasContentLength())
                    Q_EMIT q->dataReadProgress(bytesDone + q->bytesAvailable(), response.contentLength());
                else
                    Q_EMIT q->dataReadProgress(bytesDone + q->bytesAvailable(), 0);
                Q_EMIT q->readyRead(response);
            }
        }
    ...
    if (everythingRead) {
        ...
        if (response.value(QLatin1String("connection")).toLower() == QLatin1String("close")) {
            ...
            // Start a timer, so that we Q_EMIT the keep alive signal
...
/*!
    ...
    the stateChanged() signal is Q_EMITted.
